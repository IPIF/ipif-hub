...
* clone the repo
* change into the project's root directory e.g. `cd ipif-hub`
...
* `docker build -t ipifhub:latest .`
* `docker build -t ipifhub:latest --no-cache .`
...
* `docker run -it -p 8020:8020 --rm --env-file env.default --name ipifhub ipifhub:latest`
